...
class DataParallelPPOActor(BasePPOActor):
        ...
        with torch.autocast(device_type=self.device_name, dtype=self.param_dtype):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    raw_logits = logits_rmpad.gather(
                        dim=-1,
                        index=input_ids_rmpad_rolled.view(-1, 1)
                    ).squeeze(-1)  # shape: (total_nnz,)
                ...
                if self.use_ulysses_sp:
                    ...
                    raw_logits = gather_outputs_and_unpad(
                        raw_logits,
                        gather_dim=0,
                        unpad_dim=0,
                        padding_size=pad_size,
                ...
                full_raw_logits = pad_input(
                    hidden_states=raw_logits.unsqueeze(-1),
                    indices=indices,
                    batch=batch_size,
                    seqlen=seqlen,
                )
                ...
                raw_logits = full_raw_logits.squeeze(-1)[:, -response_length - 1 : -1]  # (bsz, response_length)
            ...
            return entropy, log_probs, raw_logits, self_certainty
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False, calculate_self_certainty=False) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                entropy, log_probs, raw_logits, self_certainty = self._forward_micro_batch(
            ...
            raw_logits_lst.append(raw_logits)
            if calculate_entropy:
        ...
        raw_logits = torch.concat(raw_logits_lst, dim=0)
        entropys = None
        if calculate_entropy:
        ...
        if use_dynamic_bsz:
            ...
            raw_logits = restore_dynamic_batch(raw_logits, batch_idx_list)
            if calculate_entropy:
